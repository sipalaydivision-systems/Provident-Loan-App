          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md mx-4 p-6 space-y-5">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-1">
                Upload your PDF, CSV, or Excel (.xlsx) summary file. All employees and loan data will be imported and immediately reflected in Employee Management, Loan Management, and Payments.
              </p>
            </div>
                ...
                >
                  ...
                  {importFile ? (
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-1">PDF, CSV, or Excel (.xlsx) — Provident Fund Summary format</p>
                    </div>
                  )}
                  <input ref={fileInputRef} type="file" accept=".pdf,.csv,.xlsx,.xls" className="hidden"
                  ...
                  <div className="space-y-2">
                    <div className="bg-green-50 border border-green-200 rounded-lg p-4 space-y-2">
                      ...
                      <ul className="text-sm text-green-700 space-y-0.5">
                        ...
                        <li className={importResult.loansCreated === 0 && importResult.loansUpdated === 0 ? 'text-orange-600 font-semibold' : ''}>
                          • {importResult.loansCreated ?? 0} loans created, {importResult.loansUpdated ?? 0} updated
                        </li>
                        <li>• {importResult.ledgerCreated ?? 0} payment records added</li>
                      </ul>
                    </div>
                    {(importResult.loansCreated === 0 && importResult.loansUpdated === 0) && (
                      <div className="bg-orange-50 border border-orange-200 rounded-lg p-3 text-xs text-orange-700">
                        <strong>⚠ No loan data was found in this file.</strong><br />
                        {importResult.rowsParsed > 0 && importResult.rowsWithLoanData === 0 && (
                          <span>{importResult.rowsParsed} employee rows were parsed but none had a Loan Amount value. </span>
                        )}
                        Make sure you are uploading the Provident Fund <strong>Summary</strong> file (not a Ledger Card file).
                        The file must contain columns: Loan Amount, No. of Months, Loan Balance, Status.
                      </div>
                    )}
                  </div>
                ) : (
